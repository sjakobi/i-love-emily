...
(defVar BACH-DOMINANTS ())  ; global variables for harmonic analysis. Unused here.
(defVar BACH-DOMINANTS-TONICS ())
(defVar BACH-TONICS ())

(defVar BACH-RULES ())      ; global variables holding database
...
(defVar *MIX-NAMES* ())     ; global random seed for MAKE-LEXICON-NAME. Unused.
(defVar *MIX* ())           ; global random seed for MAKE-LEXICON-NAME. Unused.

(defVar *BEAT-SIZE* 1000)   ; global constant, but usually hard-coded
...
(defVar *EVENTS* ())        ; global variable, includes composition output
(defVar *END* ())           ; local variables used during composition
(defVar *TONIC* 'major)
(defVar *EARLY-EXIT?* ())
